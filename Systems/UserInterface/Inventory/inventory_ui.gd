...
var columns: int = 8  # Number of columns in grid
var rows: int = 5  # Number of rows in grid
...
func _ready():
	...
	# Make the main Panel transparent (so only slots are visible)
	var inv_panel = $InventoryPanel
	if inv_panel:
		var transparent_style = StyleBoxFlat.new()
		transparent_style.bg_color = Color(0, 0, 0, 0)  # Fully transparent
		inv_panel.add_theme_stylebox_override("panel", transparent_style)
		# Panel should pass mouse events to children
		inv_panel.mouse_filter = Control.MOUSE_FILTER_PASS
