...
# Components
var keybinds_component: Node = null
...
func _ready():
	...
	_create_keybind_component()
	
	# Hide everything by default
	hide_all()

func _create_keybind_component():
	"""Create and initialize the keybinds component"""
	var keybinds_script = load("res://Systems/UserInterface/Debug/debug_ui_keybinds.gd")
	if keybinds_script:
		keybinds_component = Node.new()
		keybinds_component.name = "KeybindsComponent"
		keybinds_component.set_script(keybinds_script)
		add_child(keybinds_component)
		
		# Initialize the component
		keybinds_component.initialize(self, controls_ui)
	else:
...
func show_god_mode(player: Node):
	...
	# Safely get god mode constants (use defaults if not present)
	var speed_mult = player.get("GOD_SPEED_MULT") if "GOD_SPEED_MULT" in player else 2.0
	var crit_chance = player.get("GOD_CRIT_CHANCE") if "GOD_CRIT_CHANCE" in player else 1.0
	var crit_mult = player.get("GOD_CRIT_MULT") if "GOD_CRIT_MULT" in player else 5.0
	var zoom_max = player.get("god_zoom_max") if "god_zoom_max" in player else 100.0
	
	text += "Speed: x%.1f\n" % speed_mult
	text += "Crit: %.0f%% (x%.1f)\n" % [crit_chance * 100, crit_mult]
	text += "Max Zoom: %.0f\n" % zoom_max
	text += "Encumbered: OFF\n"
...
func hide_all():
	...
	enabled_label.visible = false
	god_mode_label.visible = false
	if keybinds_component:
		keybinds_component.hide()

func show_keybind_panel():
	"""Show the keybind reference panel"""
	if keybinds_component:
		keybinds_component.show()
...
func is_keybind_panel_visible() -> bool:
	...
	if keybinds_component:
		return keybinds_component.is_visible()
	return false
